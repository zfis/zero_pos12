...
class PosOrder(models.Model):
    ...
    delivery_ref = fields.Many2one('pos.delivery', string='Delivery Ref')
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            if pos_order.delivery_ref:
                pos_order.delivery_ref.write({'state': 'confirmed'})
                delivery_ids.append(pos_order.delivery_ref.id)
        ...
        return order_ids, delivery_ids

    @api.model
    def _order_fields(self, ui_order):
        order_fields = super(PosOrder, self)._order_fields(ui_order)
        delivery_id = False
        if 'delivery_ref' in ui_order:
            if ui_order['delivery_ref']:
                delivery_id = ui_order.get('delivery_ref')['id']
        order_fields['delivery_ref'] = delivery_id
        return order_fields
...
class PosDelivery(models.Model):
    ...
    @api.model
    def _order_fields(self, ui_order):
        process_line = partial(self.env['pos.delivery.line']._order_line_fields)
        return {
    ...
    date_delivery = fields.Datetime(string='Delivery Date', readonly=True, index=True, default=fields.Datetime.now)
    ...
    lines = fields.One2many('pos.delivery.line', 'order_id', string='Order Lines', copy=True)
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('pos.delivery') or '/'
        return super(PosDelivery, self).create(vals)


class PosDeliveryLine(models.Model):
    _name = "pos.delivery.line"
    ...
    order_id = fields.Many2one('pos.delivery', string='Order Ref', ondelete='cascade')
...
class PosConfig(models.Model):
    ...
    enable_delivery = fields.Boolean('Enable Delivery')
